...
import { getAllContent, getAllContentByDate, getAllContentPaths, getContent, getPrevNextContent, getTocItems } from '@/lib/content';
...
export default async function CatchAllPage({ params }: { params: Promise<{ locale: string; slug: string[] }> }) {
  ...
  if (isContentType(first)) {
    ...
    const GUIDE_STOP_WORDS = new Set(['driving', 'empire', 'voldex', 'nocturne', 'rac', 'tune', 'code', 'vehicl', 'boat', 'update', 'tier', 'guide', 'drag', 'circuit', 'atv', 'race', 'racing', 'mode', 'modes', 'list', 'list', 'wiki', 'complete', 'ultimate', 'best', 'build', 'tips', 'walkthrough', 'the', 'a', 'an', 'and', 'or', 'in', 'on', 'at', 'to', 'for', 'is', 'it', 'how', 'what', 'all', 'vs']);
    ...
    const moreGuidesSource = await getAllContentByDate(validLocale);
