...
import { VEHICLES, RACE_MODES, TIER_COLOR_MAP, TIER_LIST, TUNING_PRESETS } from '@/data/game-data';
import AdBanner from '@/components/AdBanner';
import AutoScrollCarousel from '@/components/AutoScrollCarousel';
import YouTubePlayer from '@/components/YouTubePlayer';
import { ArrowRight, ChevronRight, Car, Flag, Wrench } from 'lucide-react';
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div>
      ...
      {/* ===== VEHICLE TABLE (Data-Driven Table 1) ===== */}
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-center justify-between mb-8">
            <div>
              <div className="section-label">{t('home_module_vehicles')}</div>
              <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_vehicle_table_title')}</h2>
            </div>
            <Link href="/tier-list" title={t('home_module_tierList')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
          </div>
          <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_vehicle_table_desc')}</p>
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden backdrop-blur-sm">
            <div className="grid grid-cols-[2rem_1fr_5rem_4rem] sm:grid-cols-[2.5rem_1.5fr_6rem_5rem_5rem] gap-x-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider border-b border-[var(--color-border)] px-4 py-3">
              <span></span><span>{t('home_vehicle_table_vehicle')}</span><span>{t('tierList_tierLabel')}</span><span className="hidden sm:block">{t('home_vehicle_table_mode')}</span><span className="hidden sm:block">{t('home_vehicle_table_price')}</span>
            </div>
            {VEHICLES.map((vehicle, idx) => {
              const tc = tierColorVal(vehicle.tier);
              return (
                <Link key={vehicle.id} href="/vehicles" title={t(vehicle.nameKey)} className="grid grid-cols-[2rem_1fr_5rem_4rem] sm:grid-cols-[2.5rem_1.5fr_6rem_5rem_5rem] items-center gap-x-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                  ...
                  <span className="justify-self-start text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{vehicle.tier}</span>
                  <span className="hidden sm:block text-xs text-[var(--color-text-secondary)]">{t(vehicle.modeKey)}</span>
                  <span className="hidden sm:block text-xs text-[var(--color-text-secondary)]">{vehicle.price}</span>
                </Link>
              );
            })}
          </div>
      ...
      {/* ===== TIER LIST TABLE (Data-Driven Table 2) ===== */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="section-label">{t('home_module_tierList')}</div>
            <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_tierList_table_title')}</h2>
          </div>
          <Link href="/tier-list" title={t('home_module_tierList')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
        </div>
        <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_tierList_table_desc')}</p>
        <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden backdrop-blur-sm">
          <div className="grid grid-cols-[2rem_1fr_5rem_6rem] sm:grid-cols-[2.5rem_1.5fr_6rem_7rem_6rem] gap-x-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider border-b border-[var(--color-border)] px-4 py-3">
            <span></span><span>{t('home_vehicle_table_vehicle')}</span><span>{t('tierList_tierLabel')}</span><span className="hidden sm:block">{t('home_tierList_table_rank')}</span><span className="hidden sm:block">{t('home_vehicle_table_mode')}</span>
          </div>
          {TIER_LIST.map((entry, idx) => {
            const tc = tierColorVal(entry.tier);
            return (
              <Link key={entry.id} href="/tier-list" title={t(entry.nameKey)} className="grid grid-cols-[2rem_1fr_5rem_6rem] sm:grid-cols-[2.5rem_1.5fr_6rem_7rem_6rem] items-center gap-x-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                <span className="text-xs font-mono text-[var(--color-text-muted)]">{idx + 1}</span>
                <span className="flex items-center gap-2 font-semibold text-sm group-hover:text-[var(--color-accent)] transition-colors">
                  <Car className="w-4 h-4 text-[var(--color-accent)]" />
                  {t(entry.nameKey)}
                </span>
                <span className="justify-self-start text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{entry.tier}</span>
                <span className="hidden sm:block text-xs text-[var(--color-text-secondary)]">{t(entry.rankKey)}</span>
                <span className="hidden sm:block text-xs text-[var(--color-text-secondary)]">{t(entry.categoryKey)}</span>
              </Link>
            );
          })}
        </div>
      ...
      {/* ===== TUNING PRESETS CARDS (Data-Driven Cards 2) ===== */}
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-center justify-between mb-8">
            <div>
              <div className="section-label">{t('home_module_tuning')}</div>
              <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_tuningPresets_cards_title')}</h2>
            </div>
            <Link href="/tuning" title={t('home_module_tuning')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
          </div>
          <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_tuningPresets_cards_desc')}</p>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {TUNING_PRESETS.map((preset) => (
              <Link key={preset.id} href="/tuning" title={t(preset.nameKey)} className="category-card group block rounded-2xl overflow-hidden">
                <div className="p-5">
                  <div className="flex items-center gap-2 mb-3">
                    <Wrench className="w-4 h-4 text-[var(--color-accent)]" />
                    <span className="text-[10px] font-bold uppercase tracking-wider text-[var(--color-text-muted)]">{t(preset.modeKey)}</span>
                  </div>
                  <h3 className="text-[0.9375rem] font-bold font-[var(--font-heading)] text-[var(--color-text-primary)] group-hover:text-[var(--color-accent)] transition-colors mb-1">{t(preset.nameKey)}</h3>
                  <p className="card-meta text-[0.75rem] text-[var(--color-text-secondary)] mb-2"><span className="meta-label">{t('home_tuning_stat')}: </span><span className="meta-value font-bold text-[var(--color-accent)]">{preset.statVal}</span></p>
                  <p className="card-desc text-[0.8125rem] text-[var(--color-text-secondary)]">{t(preset.descKey)}</p>
                </div>
              </Link>
            ))}
          </div>
      ...
      {/* ===== RACE MODE CARDS (Data-Driven Cards 1) ===== */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="section-label">{t('home_module_racingModes')}</div>
            <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_raceMode_cards_title')}</h2>
          </div>
          <Link href="/racing-modes" title={t('home_module_racingModes')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
        </div>
        <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_raceMode_cards_desc')}</p>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {RACE_MODES.map((mode) => {
            const tc = mode.tier === 'S' ? 'var(--color-tier-s)' : mode.tier === 'A' ? 'var(--color-tier-a)' : mode.tier === 'B' ? 'var(--color-tier-b)' : 'var(--color-tier-c)';
            return (
              <Link key={mode.id} href="/racing-modes" title={t(mode.nameKey)} className="category-card group block rounded-2xl overflow-hidden">
                <div className="p-5">
                  <div className="flex items-center gap-2 mb-3">
                    <Flag className="w-4 h-4 text-[var(--color-accent)]" />
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{mode.tier}</span>
                  </div>
                  <h3 className="text-[0.9375rem] font-bold font-[var(--font-heading)] text-[var(--color-text-primary)] group-hover:text-[var(--color-accent)] transition-colors mb-1">{t(mode.nameKey)}</h3>
                  <p className="card-meta text-[0.75rem] text-[var(--color-text-secondary)] mb-2"><span className="meta-label">{t('home_mode_focus')}: </span><span className="meta-value">{t(mode.focusKey)}</span></p>
                  <p className="card-desc text-[0.8125rem] text-[var(--color-text-secondary)]">{t('home_mode_best')}: {t(mode.bestForKey)}</p>
                </div>
              </Link>
            );
          })}
