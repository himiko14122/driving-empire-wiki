...
const SLUG_PREFIX = 'Driving-Empire-';
// Content types derived from content/ directory dynamically
const CONTENT_ROOT = path.join(process.cwd(), 'content');
const CONTENT_TYPES = (() => {
  const enDir = path.join(CONTENT_ROOT, 'en');
  if (!fs.existsSync(enDir)) return [];
  return fs.readdirSync(enDir, { withFileTypes: true })
...
// CONTENT_TYPES is dynamically detected from content/en/ subdirectories
